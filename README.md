...
export ANTHROPIC_API_KEY=...              # judge + the API model
ollama serve && ollama pull llama3.1:8b   # open-weight models, or point base_url anywhere
p2e run --config configs/models.live.yaml --out results/live
p2e annotate --run results/live --sample  # emits the sheet you hand-label
p2e report --run results/live --annotations data/annotations/human_labels.csv
```

`configs/models.live.yaml` uses any OpenAI-compatible endpoint, which covers OpenAI,
Together, Fireworks, Groq, vLLM and Ollama. Nothing is pinned to one vendor.
...
configs/                   models.yaml (simulated, default) · models.live.yaml
